% ============================================================
%  An AI-Assisted Predictive Temporal Bridge (PTB) Architecture for
%  Virtual Real-Time Communication During Deep Space Missions
%  Target: Journal of Computer-Mediated Communication (JCMC)
%  Format: APA 7th ed. | Under 10,000 words incl. refs
% ============================================================
...
% Anonymous submission — author info in separate title page
%\title{\textbf{An AI-Assisted Predictive Temporal Bridge (PTB) Architecture for Virtual Real-Time Communication During Deep Space Missions}}
...
%Bridging the Deep Space Divide: AI-Assisted Temporal Modeling and the Illusion of Real-Time Social Presence in Interplanetary Communication.
%Simulating Synchronicity: How Predictive Temporal Bridges Mitigate the Psychological Distance of Deep Space Isolation.
%Virtually Together across Light-Minutes: Evaluating Social Presence in AI-Mediated Asynchronous Space Communication.

%Maintaining Human Bonds Across Galaxies: An AI-Mediated Approach to Pseudo-Synchronous Interaction in Deep Space Missions.
%The AI-Buffered Relationship: Using Predictive Temporal Architecture to Sustain Relational Cohesion Under Extreme Communication Latency.
%Overcoming the Interplanetary Lag: How Predictive AI Transforms Deep Space Messaging into Conversational Interaction.

%Defying the Speed of Light: Predictive AI Architecture and the Future of Social Connectedness in Deep Space.
%Ghosts in the Latency: How Predictive Temporal Bridges Simulate Real-Time Human Connection During Interplanetary Missions.
%The Predictive Bridge: Redefining Synchronicity and Social Cohesion in Deep Space Computer-Mediated Communication.
...
% ── Abstract ────────────────────────────────────────────────────────────────
\begin{abstract}
Communication latency between Earth and deep-space crewed spacecraft introduces
a fundamental breakdown in collaborative decision-making and interpersonal social presence.
For a Mars mission at median opposition, one-way signal travel time reaches
\(m \approx 240\)~seconds, rendering conventional synchronous computer-mediated
dialogue operationally and socially impossible.
This paper proposes and formally evaluates the \textit{Predictive Temporal Bridge}
(PTB)---a dual-AI architecture that transforms asynchronous, lag-driven telemetry
exchanges into a functionally synchronous social and decision-support channel.
The PTB deploys an \textit{Earth AI} that projects received spacecraft state forward
by \(2m\) seconds using physics-based predictive models, and a \textit{Cabin AI} that
reconciles Earth's forward projection against real sensor truth upon the message's
arrival.
The result is a command packet that arrives at the spacecraft at the exact moment
it was computed for, providing the Human Commander with the perceptual, psychological,
and operational experience of near-real-time collaboration.
Crucially, we ground our evaluation in computer-mediated communication (CMC) theories 
by introducing and analyzing four vital Human-Computer Interaction (HCI) metrics. 
Using a high-fidelity interactive simulation of a Mars orbital-insertion burn anomaly, 
we measure the \textit{Social Presence Index (SPI)} to quantify the crew's perception 
of ``being there'' with Ground Control; the \textit{Cognitive Load Index (CLI)} to evaluate 
operational stress during predictive desynchronization; the \textit{Conversational Latency Illusion (Lat)} 
to measure the system's effectiveness in perceptually masking the true multi-minute signal delay; 
and the \textit{Shared Situational Awareness (SA) Sync Rate} to validate the alignment between 
Earth's mental model and Mars' physical reality. Our results demonstrate that the PTB 
successfully sustains a seamless ``co-pilot'' dynamic, preventing psychological detachment, 
minimizing cognitive fatigue, and ensuring human autonomy via a robust reality-validation engine, 
thereby bridging the profound sense of isolation characterizing long-duration deep space exploration.

\smallskip
\noindent\textbf{Keywords:} deep space communication, AI-mediated communication,
social presence theory, hyperpersonal model, ethics of mediation, Mars mission,
situational awareness, human-AI teaming
\end{abstract}
...
one-way, with a median value near 240~seconds \citep{nasa_ola_2023}.
...
relational maintenance and mutual responsiveness are essential \citep{walther1992}.
...
temporal displacement and isolation from the human species \citep{kanas2017}.
\citet{diamond2025} demonstrated in analog Mars missions that communication
...
Conversely, \citet{mavrakis2025} showed that integrating human-in-the-loop AI
...
\citep{kanas2017}. Redesigning the \textit{computer-mediated interface} to create an
...
Following the foundational agenda of \citet{hancock2020}, we define AI-MC as communication
...
Social Presence Theory \citep{short1976} posits that communication media vary in their
...
cognitive friction \citep{walther1996}. In deep-space environments, this latency is
...
and feedback that binds collaborative teams \citep{walther1992}.
...
The Hyperpersonal Model of CMC \citep{walther1996} argues that computer-mediated environments
...
concerning agency and automation bias \citep{hancock2020}.
...
\citet{endsley1995} defines situational awareness (SA) as the perception, comprehension,
...
mental model of the system's current and future states \citep{salas1995}.
...
\subsection{Methodology and Scenario Design}
...
\subsection{Web Dashboard Visual Architecture}
...
\subsection{Human-Computer Interaction (HCI) Metrics Formulation}
...
\subsection{Reduction in Decision Latency}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{fig_latency_illusion.pdf}
...
\subsection{Psychological and Social Benefits}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{fig_hci_metrics.pdf}
...
This is the operational fulfillment of the "Mission Facilitation" model \citep{mccann2006}.
