...
The psychological and operational impacts of deep-space communication latency have been extensively documented. Recent studies by \cite{diamond2025} highlight how multi-minute delays between Mars and Earth induce severe mission controller workload, cognitive friction, and interpersonal stress, severely hindering collaborative decision-making. 

In the domain of communication networks, recent literature emphasizes AI-driven enhancements to traditional Delay-Tolerant Networking (DTN). For instance, \cite{jsiar2024} demonstrated that integrating machine learning into hybrid interplanetary satellite architectures can significantly optimize link stability and routing over disrupted channels. Furthermore, cognitive communication systems \cite{nasa_cognitive2023} are being explored to autonomously predict channel conditions and manage spectrum usage, effectively bypassing manual calculations that contribute to operational delay. Within aerospace hardware research, predictive edge AI is increasingly applied to physical space systems, such as compensating for actuator delays in deep-space laser communications \cite{acceleron2024}. While these network and hardware-level AI integrations successfully maximize physical data throughput, they do not resolve the cognitive latency experienced by human crews.

To address these latency challenges... the aerospace community has traditionally relied on network-level routing paradigms or predictive mechanical displays. For instance, the Smith Predictor \cite{sheridan1993} and NASA JPL's ``phantom robot'' \cite{bejczy1990} successfully close the mechanical control loop in robotic teleoperation by overlaying predicted kinematic states over delayed video feeds. However, these classical systems treat the human operator in isolation, failing to address the social and conversational desynchronization experienced by human crews.

More recently, research has shifted toward algorithmic mediation. \cite{mavrakis2025} proposed integrating human-in-the-loop AI to tackle space communication challenges, emphasizing the need for intelligent systems that assist rather than replace human operators. Parallel to this, the broader field of AI-Mediated Communication (AI-MC) \cite{hancock2020} explores how intelligent agents can alter interpersonal interactions. While current AI-MC applications primarily focus on content modification (e.g., smart replies) in near-instantaneous networks, the Predictive Temporal Bridge diverges by utilizing AI to synchronize the temporal \textit{delivery} and \textit{context} of information, bridging the physical light-time gap to restore interpersonal co-presence.
...
for deep space operations (see Figure~\ref{fig:path_error}).
...
present on their console, reducing the perceived operational latency to zero (Figure~\ref{fig:latency_illusion}).
...
Figure~\ref{fig:hci_metrics} tracks these dynamic responses.
...
\subsubsection{Operational Paradigm Shift: Ground as a Predictive Cloud}
...
\subsubsection{AI-Mediated Communication Design Principles for Extreme Latency}
